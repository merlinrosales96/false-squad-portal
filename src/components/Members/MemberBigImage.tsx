        ...
        {members.map(({ id, name }) => (
          <Box key={id} className="relative h-20 overflow-hidden">
            <img
              ...
              src={`/images/members/text/${id}.webp`}
              alt={name}
              decoding="async"
            ...
            <Box
              ...
              className="absolute inset-0 -translate-x-full rotate-45 bg-gradient-to-tr from-transparent via-pink-400 to-transparent transition-transform duration-[2s] ease-in-out"
            />
          </Box>
        ))}
      </Box>
      ...
      <Box className="mask-fade-bottom relative bottom-0 mx-auto flex h-[80vh] w-full items-center justify-center">
        {members.map(({ id, name }) => (
          <img
            ...
            src={`/images/members/big/${id}.webp`}
            alt={name}
            decoding="async"
